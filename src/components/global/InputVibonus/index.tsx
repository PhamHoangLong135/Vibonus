...
const InputVibonus = () => {
  ...
  useEffect(() => {
    ...
    dispatch({ type: GET_TAG_LIST_REQUEST });
  }, [dispatch]);

  const [prefix, setFrefix] = useState("@");
  const searchusername = (text: string, trigger: string) => {
    setFrefix(trigger);
  };
  const {
    handleSubmit,
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div style={{ marginBottom: 24 }}>
          <ul className="tabs">
            <NavLink to="/home">
              <button className="parallenlogram select" id="btnVibonus">
                <AdjustIcon />
                <div className="txtParallenlogram">Vibonus</div>
              </button>
            </NavLink>
            <NavLink to="/home/viforum">
              <button className="parallenlogram " id="btnVinofurm">
                <div className="txtViforum">
                  <div>Viforum</div>
                </div>
              </button>
            </NavLink>
          ...
          <Paper sx={{ padding: 2 }}>
            <Avatar src={usersId.usersId?.avatar}></Avatar>
            <Controller
              name="title"
              control={control}
              render={({ field }) => (
                <Mentions
                  ...
                  prefix={["@", "#"]}
                  placeholder="+5 @Username #tags tks for sp"
                >
                  {prefix === "@"
